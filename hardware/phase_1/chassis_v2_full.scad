...
//   - Pololu #1995 bracket dims (BRACKET_FACE_T, BRACKET_PLATE_W/H, BRACKET_BOLT_R)
//     → get actual drawing from pololu.com/product/1995 (PDF linked on product page)
...
//   - Confirm Pololu #1995 bracket plate dims (BRACKET_PLATE_W/H) and BRACKET_BOLT_R from drawing PDF
//
// ============================================================================
...
//                                                                  6×M3 face mounting | ⌀31mm bolt circle (R=15.5mm)
//                             Options: 19:1=52mm  30:1=56mm  50:1=57mm body length
//
//  2   4   Motor bracket      Pololu #1995 Machined Aluminum       Face-mount plate (NOT L-bracket)
//                             Bracket for 37D mm Gearmotors       6× M3 to gearbox face (R=15.5mm circle)
//                                                                  3× M3 tapped to bottom deck (14.8mm spacing)
//
...
MOTOR_DECK_GAP         =  5;    // Gap: motor top / bracket top-edge to deck underside
                                 // M3 screws (deck 8mm + gap 5mm + 6mm thread) → M3×20 from above
...
MOTOR_L           = 52;   // = motor can (30.7) + gearbox body L; 19:1/30:1=52.2mm, 50:1/70:1=54.7mm - UPDATE when ratio chosen
                           // Does NOT include output section (22mm) or encoder (15.4mm)
...
// Overall plate dims and BRACKET_FACE_T are ESTIMATED — verify from product drawing PDF.
BRACKET_FACE_T          =  6;    // Bracket body depth along motor axis (ESTIMATED; machined piece)
BRACKET_PLATE_W         = 37;    // Bracket plate width (Y, ~= 37D face dia) (ESTIMATED)
BRACKET_PLATE_H         = 37;    // Bracket plate height (Z, ~= 37D face dia) (ESTIMATED)
BRACKET_BOLT_R          = 15.5;  // 37D gearbox face bolt circle radius (from Pololu drawing); 6× M3
BRACKET_SHAFT_D         = 10;    // Shaft clearance hole dia in bracket face (ESTIMATED)
...
// ESTIMATED overall dims (BRACKET_FACE_T, BRACKET_PLATE_W, BRACKET_PLATE_H) —
//   verify from pololu.com/product/1995 drawing PDF before printing.
...
module motor_bracket() {
    ...
    difference() {
        // Plate body
        translate([MOTOR_L, -BRACKET_PLATE_W/2, -BRACKET_PLATE_H/2])
...
module drivetrain_stack() {
    ...
    // Motor face-mount bracket (Pololu #1995) — ESTIMATED dims, see motor_bracket module
    motor_bracket();
...
// Decks
// Bottom deck: 20×24mm slot (axle×collar), 26×30mm flange stop, 216×216mm plate (2mm margin on Core One 220mm Y)
// Bracket holes: 3× M3 clearance per motor (4 motors = 12 holes) at X=±BRACKET_DECK_X=±63mm.
//   Screws pass through deck from above and thread into bracket's tapped M3 holes (M3×20 recommended).
//   Hole Y positions: strut_Y ± {0, ±BRACKET_MOUNT_SPACING} = {0, ±14.8mm} from each strut centre.
color("SlateGray", 0.6)
difference() {
    ...
    // Motor bracket mounting holes — M3 clearance from top face through deck
    for (sx = [-1, 1])              // left / right motor pairs
        for (sy = [0, -STRUT_Y])    // front / rear strut Y positions
            for (dy = [-BRACKET_MOUNT_SPACING, 0, BRACKET_MOUNT_SPACING])
